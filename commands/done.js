        ...
        if(message.channel.id === "814074843653079050"){
          ...
          todoDB.get(message.author.id).then(document => {
            for(let i = 0; i < nbrArray.length; i++) {
              if(utils.indexOutOfBound(document["tasksDone"], nbrArray[i])){
                ...
                for(let j = 0; j < nbrArray.length; j++){
                  document["tasksDone"][nbrArray[j]-1] = true;//-1 because the numbers in the todo list are displayed from 1 onwards.
                }
